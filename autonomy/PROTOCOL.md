...
driver's own handoff notes. The machinery was ported from `ws/windows`
(archived in `autonomy/windows/`, with the other merged branches' drivers).
...
- Stay on branch `ws/mergin` in this worktree. Never switch, create or
...
- Commit messages follow the repo's GSD style (`feat(02.8-01): …`,
  `docs(02.8): …`) and end with
