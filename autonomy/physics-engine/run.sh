#!/usr/bin/env bash
# autonomy/run.sh — unattended driver for the mathspace build.
#
# Each iteration starts a FRESH claude session (fresh context), hands it
# autonomy/PROMPT.md plus a time-limit line, and lets it do one bounded
# slice of work per the protocol in CLAUDE.md. Memory between sessions is
# autonomy/STATE.md plus git history, so context size never becomes a
# problem: no session lives long enough to fill it.
#
...
#
# Stops when autonomy/DONE exists (a session writes it once every plan
# phase is done), when autonomy/STOP exists, when MAX_SESSIONS is reached,
# or on Ctrl-C. STALL_LIMIT sessions in a row with no new commit switches
# to a long backoff instead of stopping, so a transient failure does not
# burn the night; STALL_STOP in a row stops the loop, because a run that
# commits nothing for that long is either stuck or out of work.
#
# Usage:
#   autonomy/run.sh                 # run until DONE
#   MAX_SESSIONS=2 autonomy/run.sh  # a short trial
...
#   STALL_STOP       no-commit sessions before stopping, 0 = never (default: 6)
...
while :; do
    ...
    if [ "$STALL_STOP" -gt 0 ] && [ "$stall" -ge "$STALL_STOP" ]; then
        echo "run.sh: $stall sessions in a row with no commit; stopping. Read the last logs and autonomy/STATE.md (Blocked, Next)."
        break
    fi
