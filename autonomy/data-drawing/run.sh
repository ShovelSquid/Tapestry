#!/usr/bin/env bash
# autonomy/run.sh — unattended driver for the Data Drawing GSD phases on this worktree.
#
# Ported from ws/windows (itself from ws/physics-engine). Each iteration starts a FRESH claude session
...
# The GSD project (.planning/, .gsd/, .claude/) lives in data-drawing/, so the
# sessions start there; git and the autonomy/ files are reached from $ROOT.
GSD_DIR="$ROOT/data-drawing"
mkdir -p "$LOG_DIR"
