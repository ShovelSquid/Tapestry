...
import type { ExpectedTree, OpenTree, TreeEntry, TreeRegistry, UnavailableTree } from '../trees/registry'
...
export class WorkspaceService implements WorkspaceLookup {
  ...
  async openWorkspace(
    ...
    treePath: string,
    expect?: ExpectedTree,
  ): Promise<OpenTree | UnavailableTree> {
    const target = resolve(root)
    let name = target.slice(target.lastIndexOf('/') + 1)
    // `expect` is the forest's recorded digest (2.6 D-03): a different world
    // at the remembered path is listed unavailable rather than adopted.
    const opts = {
