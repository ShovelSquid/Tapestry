...
int main(int argc, char** argv) {
    ...
    // ---- constraint fixtures ------------------------------------------------
    {
        std::string text = header(
            "pendulum: a static anchor (particle 1) at the origin and a mass-1 bob (particle 2) at (2, 0), "
            "joined by a stiffness-1 rest-length-2 constraint (constraint 1) at tick 0; no strokes, no brushes");
        text += "seed 1\n";
        const std::int64_t rest = ddsim::fx64::ONE * 2;
        text += actionLine(0, ddsim_test::encodeCreateParticle(1, true, 0, 0, 0));
        text += actionLine(0, ddsim_test::encodeCreateParticle(2, false, ddsim::fx64::ONE, rest, 0));
        text += actionLine(0, ddsim_test::encodeCreateConstraint(1, 1, 2, rest, ddsim::fx64::ONE));
        text += checkpoints({0, 1, 60, 300, 600});
        if (!emit(dir, "pendulum", text)) return 1;
    }
    {
        std::string text = header(
            "rope-chain: a static anchor (particle 1) at the origin and two mass-1 links (particles 2, 3) at "
            "(2, 0) and (4, 0), joined by two stiffness-1 rest-length-2 constraints (1-2, 2-3) at tick 0; no "
            "strokes, no brushes");
        text += "seed 1\n";
        const std::int64_t rest = ddsim::fx64::ONE * 2;
        text += actionLine(0, ddsim_test::encodeCreateParticle(1, true, 0, 0, 0));
        text += actionLine(0, ddsim_test::encodeCreateParticle(2, false, ddsim::fx64::ONE, rest, 0));
        text += actionLine(0, ddsim_test::encodeCreateParticle(3, false, ddsim::fx64::ONE, rest * 2, 0));
        text += actionLine(0, ddsim_test::encodeCreateConstraint(1, 1, 2, rest, ddsim::fx64::ONE));
        text += actionLine(0, ddsim_test::encodeCreateConstraint(2, 2, 3, rest, ddsim::fx64::ONE));
        text += checkpoints({0, 1, 60, 300, 600});
        if (!emit(dir, "rope-chain", text)) return 1;
    }

    std::printf("wrote %s/{noop,one-brush,many-brushes,one-stroke,four-per-tick,gap,two-strokes,two-strokes-inserted,brush-edit,presets,pendulum,rope-chain}.actions\n",
                dir.c_str());
    return 0;
}
