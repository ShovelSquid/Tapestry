...
// ParticleId and ConstraintId are assigned sequentially (1, 2, 3, ...), the
// same rule BrushVersionId already follows: id N is always stored at index
// N - 1, so storage order is id order with a plain push_back, never a
// lower_bound insert like ActiveStroke needs.
struct ParticleId {
    std::uint32_t value = 0;

    constexpr bool assigned() const { return value != 0; }

    friend constexpr bool operator==(ParticleId a, ParticleId b) { return a.value == b.value; }
    friend constexpr bool operator!=(ParticleId a, ParticleId b) { return a.value != b.value; }
    friend constexpr bool operator<(ParticleId a, ParticleId b) { return a.value < b.value; }
};
...
struct ConstraintId {
    ...
    friend constexpr bool operator==(ConstraintId a, ConstraintId b) { return a.value == b.value; }
    friend constexpr bool operator!=(ConstraintId a, ConstraintId b) { return a.value != b.value; }
    friend constexpr bool operator<(ConstraintId a, ConstraintId b) { return a.value < b.value; }
};
