...
std::string plotText(const std::vector<FixtureAction>& log) {
    ...
    for (const std::uint64_t t : {0u, 1u, 4u, 8u}) {
...
TEST_CASE("golden plot: fixture equals the generator, bound fields evaluate") {
    ...
    const std::string expected = plotText(log);
    const std::string path = goldenPath("plot", "actions");
    if (std::getenv("MS_WRITE_FIXTURES") != nullptr) {
        REQUIRE(writeFile(path, expected));
    }
    CHECK_MESSAGE(readFile(path) == expected, "plot.actions differs from plotLog(); rerun with MS_WRITE_FIXTURES=1");
