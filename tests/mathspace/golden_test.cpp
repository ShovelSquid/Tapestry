...
// Phase 7 (7b): the brush body preset rule. A 2-space, body note 2 placed on
// its first sample at rest with k = 1/64 (brush mass 64, the heavy
// underdamped brush), rule 3 with the brush force. The pen moves the target
// at ticks 1, 5, 10 and 20 and then pauses: the body keeps moving after the
// pen stops (that is the momentum), so checkpoints reach 40. The checkpoint
// callback walks ddsim's body_substep alongside and expects raw equality.
namespace {
constexpr const char* BRUSH_FORCE_EXPR = "self.k * (self.target - self.pos) - sqrt(self.k) * self.velocity";
constexpr std::uint64_t BRUSH_SAMPLE_TICKS[4] = {1, 5, 10, 20};
constexpr std::int32_t BRUSH_SAMPLES[4][2] = {{40, 0}, {40, 30}, {0, 30}, {0, 0}};
}

std::vector<FixtureAction> brushLog() {
    const NoteId space{1};
    const NoteId body{2};
    const NoteId rule{3};
    World w(7);
    ...
    add(0, encode_create_space(space, 2));
    add(0, encode_create_note(body, space_of(space), NoteKind::Note));
    add(0, encode_set_field(body, pos2(0, 0)));
    add(0, encode_set_field(body, vec2(VELOCITY_FIELD, 0, 0)));
    add(0, encode_set_field(body, vec2("target", 0, 0)));
    Field k;
    k.name = "k";
    k.value[0] = fx64::from_int(1) / fx64::from_int(64);
    add(0, encode_set_field(body, k));
    add(0, encode_create_note(rule, space_of(space), NoteKind::Rule));
    add(0, encode_bind_field(rule, FORCE_FIELD, rule_code_for(w, rule, BRUSH_FORCE_EXPR)));
    for (int i = 0; i < 4; ++i) {
        add(BRUSH_SAMPLE_TICKS[i], encode_set_field(body, vec2("target", BRUSH_SAMPLES[i][0], BRUSH_SAMPLES[i][1])));
    }
    return log;
}

TEST_CASE("golden brush: fixture equals the generator, the body follows ddsim's body_substep") {
    const std::vector<FixtureAction> log = brushLog();
    checkFixtureText("brush",
                     fixtureText("# brush — phase 7 brush body rule: a 2-space (id 1), body note 2 at rest on\n"
                                 "# its first sample with k = 1/64 (brush mass 64), rule 3 with force\n"
                                 "# k (target - pos) - sqrt(k) velocity. The target moves at ticks 1, 5, 10\n"
                                 "# and 20, then the pen pauses and the body keeps moving. Generated by\n"
                                 "# golden_test.cpp's brushLog (MS_WRITE_FIXTURES=1 rewrites this file).\n",
                                 log, 7, {0, 1, 5, 10, 20, 40}));
    const Fixture f = load("brush");
    REQUIRE(f.actions.size() == log.size());
    for (std::size_t i = 0; i < log.size(); ++i) {
        CHECK(f.actions[i].tick == log[i].tick);
        CHECK_MESSAGE(hex(f.actions[i].bytes) == hex(log[i].bytes), "action " << i);
    ...
    World w(f.seed);
    REQUIRE(replayFixture(f, w, [&](std::uint64_t tick, const World& at) {
        for (; dd_tick < tick; ++dd_tick) {
            for (int i = 0; i < 4; ++i) {
                if (BRUSH_SAMPLE_TICKS[i] == dd_tick) {
                    tu = fx64::from_int(BRUSH_SAMPLES[i][0]);
