/**
 * engine-core.js — the Engine class over the flat C ABI, with nothing
 * Node-specific in it.
 *
 * engine.js adds the Node loader (paths, file URLs) and is what the host
 * and the runner require(); the stage surface bundles this file into the
 * renderer and loads the glue itself, so anything that touches `node:`
 * must stay out of here. Bytes cross the boundary through _malloc/_free;
 * HEAPU8 is re-read from the module at every use because
 * ALLOW_MEMORY_GROWTH can replace it.
 *
 * Nothing here converts numbers: actions arrive already encoded (image.js
 * does real↔fx64 exactly) and the snapshot comes back as raw bytes.
 */
