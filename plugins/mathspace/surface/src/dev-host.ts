 ...
 * whose kernel holds one fixture world (a 3D space, two views, two notes),
 * so the surface can be tried without the app. `window.__ms.handle` is the
 * mounted surface for console poking (`await __ms.handle.refresh()`).
 */
...
const handle: MountedSurface = mount(host)
declare global {
  interface Window {
