...
// emission rules for every active stroke in ordinal order, then the
// particle/constraint rule (rules/constraints.hpp) over every particle and
// constraint, then advances the tick; it is the only thing that advances
// the tick and takes no time argument, so one step per call and four per
// burst are the same sim. The snapshot byte buffers handed out through the
// C ABI are rebuilt field by field after every accepted change.
...
#include "ddsim/rules/constraints.hpp"
...
int Sim::apply(const std::uint8_t* action, std::uint32_t len) {
    ...
    case ActionKind::CreateParticle: {
        CreateParticle in;
        const int rc = decode_create_particle(r, in);
        if (rc != DD_OK) {
            return rc;
        }
        if (!r.at_end()) {
        ...
        if (in.particle_id != state_.particles.size() + 1) {
            return DD_ERR_PARTICLE_ID;
        }
        if (state_.particles.size() >= DD_MAX_PARTICLES) {
            return DD_ERR_LIMIT;
        }
        Particle p;
        p.id.value = in.particle_id;
        p.is_static = in.is_static;
        p.mass = in.mass;
        p.x = in.x;
        p.y = in.y;
        p.vx = fx64::from_raw(0);
        p.vy = fx64::from_raw(0);
        const int vr = validate_particle(p);
        if (vr != DD_OK) {
            return vr;
        }
    ...
    case ActionKind::CreateConstraint: {
        CreateConstraint in;
        const int rc = decode_create_constraint(r, in);
        if (rc != DD_OK) {
            return rc;
        }
        if (!r.at_end()) {
        ...
        if (in.constraint_id != state_.constraints.size() + 1) {
            return DD_ERR_CONSTRAINT_ID;
        }
        if (state_.constraints.size() >= DD_MAX_CONSTRAINTS) {
            return DD_ERR_LIMIT;
        }
        if (in.particle_a > state_.particles.size() || in.particle_b > state_.particles.size()) {
            return DD_ERR_PARTICLE_ID;
        }
        Constraint c;
        c.id.value = in.constraint_id;
        c.particle_a = in.particle_a;
        c.particle_b = in.particle_b;
        c.rest_length = in.rest_length;
        c.stiffness = in.stiffness;
        const int vr = validate_constraint(c);
        if (vr != DD_OK) {
            return vr;
        }
        state_.constraints.push_back(c);
        refresh_snapshots();
        return DD_OK;
    }
    default:
